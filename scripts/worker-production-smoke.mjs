...
assert.equal(health.aiConfigured, true, 'Workers AI binding is configured');

const options = await fetch(`${workerUrl}/api/coach/interpret`, {
  method: 'OPTIONS',
...
const valid = await fetch(`${workerUrl}/api/coach/interpret`, {
  method: 'POST',
...
const invalid = await fetch(`${workerUrl}/api/coach/interpret`, {
  method: 'POST',
  headers: { Origin: frontendOrigin, 'Content-Type': 'application/json' },
...
const blocked = await fetch(`${workerUrl}/api/coach/interpret`, {
  method: 'POST',
