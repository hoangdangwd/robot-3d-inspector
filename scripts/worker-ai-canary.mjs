...
if (process.env.RUN_AI_CANARY !== '1') {
  console.log('SKIP: AI canary disabled; set RUN_AI_CANARY=1 to exercise Workers AI output.');
  process.exit(0);
}
...
const response = await fetch(`${workerUrl}/api/coach/interpret`, {
  method: 'POST',
...
assert.equal(payload?.source, 'workers_ai', 'canary must exercise the model path');
...
if (response.status === 200) {
  ...
  console.log(`PASS: Workers AI canary returned a constrained ${payload.intent.kind} for ${workerUrl}.`);
} else {
  assert.equal(payload?.error?.code, 'MODEL_UNRECOGNIZED', 'unrecognized model output is a safe non-fatal result');
  console.log(`PASS: Workers AI canary exercised the model path; provider returned safe MODEL_UNRECOGNIZED for ${workerUrl}.`);
}
